"""
    test_dcc_vs_cc_unicycle_sym.jl

Compares CC vs DCC encoding on the Unicycle benchmark using hybrid symbolic
reachability (multi_step_hybreach), which is harder than concrete reachability
and more likely to reveal LP-relaxation differences between CC and DCC.

Metrics: wall-clock time, reachset volume, MIP size (binary vars, linear constraints).

Run from repo root:
    julia --project=. tests/test_dcc_vs_cc_unicycle_sym.jl
"""
...
concInt      = [2, 2]   # hybrid intervals: two concrete+symbolic passes of 2 steps each
N_overt      = 2
...
function bound_unicycle(Unicycle; npoint=2)
    ...
    l_x1Sub1LB, l_x1Sub1UB = lift_OA(emptyList, currList, x1FuncSub_1LB, x1FuncSub_1UB, lbs[3:4], ubs[3:4])
    emptyList = [2]; currList = [1]
    l_x1Sub2LB, l_x1Sub2UB = lift_OA(emptyList, currList, x1FuncSub_2LB, x1FuncSub_2UB, lbs[3:4], ubs[3:4])
    ...
    emptyList = [1]; currList = [2]
    l_x2Sub1LB, l_x2Sub1UB = lift_OA(emptyList, currList, x2FuncSub1LB, x2FuncSub1UB, lbs[3:4], ubs[3:4])
    emptyList = [2]; currList = [1]
    l_x2Sub2LB, l_x2Sub2UB = lift_OA(emptyList, currList, x2FuncSub2LB, x2FuncSub2UB, lbs[3:4], ubs[3:4])
...
# ── MIP size helper (summed over the full symbolic graph) ─────────────────────
...
# ── Run one hybrid symbolic reach and collect metrics ─────────────────────────
...
function run_encoding(enc_func, label)
    ...
    println("  Final set bounds : low=$(round.(low(final_set), digits=5)), high=$(round.(high(final_set), digits=5))")
...
println("\nUnicycle benchmark: CC vs DCC encoding (hybrid symbolic reachability)")
...
println("="^50)
println("                   CC          DCC")
println("  Total time (s): $(lpad(round(results_cc.time,  digits=2), 8))    $(lpad(round(results_dcc.time,  digits=2), 8))")
...
if vol_diff < 1e-4
    println("  [PASS] Reachsets match — DCC is sound.")
else
