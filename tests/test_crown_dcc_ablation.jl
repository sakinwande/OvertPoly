"""
    test_crown_dcc_ablation.jl

Runs CROWN + DCC encoding on TORA (20 concrete steps) and Unicycle
(concInt=[10,10,10] hybrid steps). No Anderson cuts.

Run from repo root:
    julia --project=. tests/test_crown_dcc_ablation.jl
"""
...
println("\n", "="^60)
println("CROWN+DCC Ablation")
println("="^60)

println("\n--- TORA: CROWN_DCC (warmup) ---")
q_warm = GraphPolyQuery(TORA, tora_controller, Id(), "MIP", 2, tora_dt, 2, nothing, nothing, 2)
multi_step_concreach(q_warm; enc_func=dccEncoding!, use_crown=true)

println("\n--- TORA: CROWN_DCC (timed, $TORA_STEPS steps) ---")
TORA.domain = TORA_DOMAIN
q_tora = GraphPolyQuery(TORA, tora_controller, Id(), "MIP", TORA_STEPS, tora_dt, 2, nothing, nothing, 2)
t0 = time()
tora_sets, _ = multi_step_concreach(q_tora; enc_func=dccEncoding!, use_crown=true)
tora_time = time() - t0
tora_vol  = volume(tora_sets[end])
println("  Time:   $(round(tora_time, digits=2)) s  ($(round(tora_time/TORA_STEPS, digits=2)) s/step)")
println("  Volume: $tora_vol")
println("  Low:    $(round.(low(tora_sets[end]),  digits=4))")
println("  High:   $(round.(high(tora_sets[end]), digits=4))")

# ── Unicycle: CROWN + DCC ─────────────────────────────────────────────────────

println("\n--- Unicycle: CROWN_DCC (warmup) ---")
Unicycle.domain = UNI_DOMAIN
q_warm_uni = GraphPolyQuery(Unicycle, uni_controller, Id(), "MIP", sum([2,2]), uni_dt, 2, nothing, nothing, 2)
multi_step_hybreach(q_warm_uni, uni_depMat, [2, 2]; enc_func=dccEncoding!, use_crown=true)

println("\n--- Unicycle: CROWN_DCC (timed, concInt=$UNI_CONC_INT) ---")
Unicycle.domain = UNI_DOMAIN
q_uni = GraphPolyQuery(Unicycle, uni_controller, Id(), "MIP", sum(UNI_CONC_INT), uni_dt, 2, nothing, nothing, 2)
t0 = time()
uni_sets, _ = multi_step_hybreach(q_uni, uni_depMat, UNI_CONC_INT; enc_func=dccEncoding!, use_crown=true)
uni_time = time() - t0
uni_vol  = volume(uni_sets[end])
println("  Time:   $(round(uni_time, digits=2)) s")
println("  Volume: $uni_vol")
println("  Low:    $(round.(low(uni_sets[end]),  digits=4))")
println("  High:   $(round.(high(uni_sets[end]), digits=4))")
...
println("Summary")
println("="^60)
println(@sprintf("%-12s  %-14s  %9s  %8s", "Benchmark", "Config", "Time (s)", "Volume"))
println("-"^50)
println(@sprintf("%-12s  %-14s  %9.2f  %8.5f", "TORA (20 st)", "CROWN_DCC", tora_time, tora_vol))
println(@sprintf("%-12s  %-14s  %9.2f  %8.5f", "Unicycle(30)", "CROWN_DCC", uni_time, uni_vol))

# ── Write report ──────────────────────────────────────────────────────────────

ts = Dates.format(now(), "yyyy-mm-dd-HH-MM")
report_path = joinpath(@__DIR__, "reports", "report_crown_dcc_ablation_$(ts).md")

open(report_path, "w") do io
    println(io, "# CROWN+DCC Ablation — $(ts)")
    println(io, "")
    println(io, "## Setup")
    println(io, "- Encoding: DCC (disaggregated convex combination)")
    ...
    println(io, "|-----------|--------|----------|--------|")
    println(io, @sprintf("| TORA (20 steps, dt=0.1) | CROWN_DCC | %.2f | %.5f |", tora_time, tora_vol))
    println(io, @sprintf("| Unicycle (concInt=%s, dt=0.2) | CROWN_DCC | %.2f | %.5g |", string(UNI_CONC_INT), uni_time, uni_vol))
    println(io, "")
    println(io, "## Final reachset bounds")
    println(io, "")
    println(io, "**TORA:** low=$(round.(low(tora_sets[end]), digits=4)), high=$(round.(high(tora_sets[end]), digits=4))")
    println(io, "")
    println(io, "**Unicycle:** low=$(round.(low(uni_sets[end]), digits=4)), high=$(round.(high(uni_sets[end]), digits=4))")
end
