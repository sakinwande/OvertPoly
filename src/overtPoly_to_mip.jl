...
zUB = [tup[end] for tup in UB]
zLB = [tup[end] for tup in LB]

lbPWL = BivariatePWLFunction(xS, zLB, Tri)
ubPWL = BivariatePWLFunction(xS, zUB, Tri)
...
# z = piecewiselinear(model, x[1], x[2], lbPWL)
...
function all_variables(model::Model)
    return [Variable(model, idx) for idx in MOI.get(model, MOI.ListOfVariableIndices())]
end
