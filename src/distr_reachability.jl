...
function encode_dynamics!(query::GraphPolyQuery; enc_func=ccEncoding!)
    ...
    for sym in query.problem.varList
        ...
        dynNodes[ind] = enc_func(xS, yLB, yUB, Tri, query, query.problem.varList[ind], ind, dynNodes[ind])
    end
...
function concreach!(query::GraphPolyQuery; digits=15, enc_func=ccEncoding!, use_crown::Bool=true,
                    use_anderson::Bool=false, max_cuts_per_layer::Int=typemax(Int))
    ...
    encode_dynamics!(query; enc_func=enc_func)

    #Encode the network and link the control to the dynamics
    if !isnothing(query.network_file)
        neurons = encode_control!(query; use_crown=use_crown, use_anderson=use_anderson,
                                         max_cuts_per_layer=max_cuts_per_layer)
    end
...
function multi_step_concreach(query::GraphPolyQuery; digits=15, enc_func=ccEncoding!, use_crown::Bool=true,
    ...
    for i = 1:query.ntime
        ...
        reachSet, boundSet = concreach!(query; digits=digits, enc_func=enc_func,
                                         use_crown=use_crown, use_anderson=use_anderson,
                                         max_cuts_per_layer=max_cuts_per_layer)
...
function encode_sym_dynamics!(symQuery::GraphPolyQuery, x_dim; enc_func=ccEncoding!)
    """
    ...
    for t_ind = 1:symQuery.ntime
        ...
        for (x_ind, sym) in enumerate(symQuery.problem.varList)
            ...
            enc_func(xS, yLB, yUB, Tri, symQuery, sym_t, x_ind, dynNodes[x_ind])
        end
...
function hybreach(symQuery::GraphPolyQuery, depMat, t_sym, reachSets=nothing, boundSets=nothing; enc_func=ccEncoding!, use_crown::Bool=true,
    ...
    if isnothing(boundSets)
        ...
        reachSets, boundSets = multi_step_concreach(concQuery; enc_func=enc_func, use_crown=use_crown,
                                                     use_anderson=use_anderson, max_cuts_per_layer=max_cuts_per_layer)
    end
